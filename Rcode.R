...
# a) Load packages
# load packages
library(mlbench)#A collection of artificial and
#real-world machine learning benchmark problems
#, including, e.g., several data sets from the UCI repository.
library(caret)#short for Classification And REgression Training)
#caret is a set of functions that attempt to streamline the process
#for creating predictive models. The package contains tools for:data splitting
#pre-processing,feature selection,model tuning using re sampling,variable importance estimation

library(corrplot) #corr plot provides a visual exploratory 
#tool on correlation matrix that supports automatic variable reordering 
library(vtable) # serves the purpose of outputting automatic variable documentation 
#that can be easily viewed while continuing to work with data.
# b) Load dataset
...
set.seed(4444) # we set seed so that we get the same results for randomization
...
summary(dataset)
#to ctreate a table of the summary statistics
st(BostonHousing)
...
missmap(BostonHousing, col=c("black", "grey"), legend=TRUE)
...
dim(datasetFeatures)
datasetFeatures <- dataset[,-8] #removing the b variable as well


#boxcox and removing the hghly corelated predictiors
...
#In this section we will look at tuning the parameters
#of SVM with a Radial Basis Function (RBF).
print(fit.svm)
fit.svm
